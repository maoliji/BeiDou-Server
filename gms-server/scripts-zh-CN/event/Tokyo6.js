...
var entryMap = 802000801;    // 战斗地图
var exitMap = 802000800;     // 退出后返回地图
var recruitMap = 802000800;  // 远征队招募地图（NPC所在）
var clearMap = exitMap;      // 通关后返回地图

var minMapId = 802000801;
var maxMapId = 802000820;
...
function setup(channel) {
    ...
    // 修正：使用 eim.getInstanceMap 获取地图并重置
    var mapIds = [802000801, 802000802, 802000803, 802000804, 802000805, 802000806];
    for (var i = 0; i < mapIds.length; i++) {
        var map = eim.getInstanceMap(mapIds[i]);
        if (map != null) {
            map.resetFully();
        }
    }
...
function clearPQ(eim) {
    ...
    eim.dropMessage(5, "[远征队] 恭喜！你们成功击败了欧碧拉！");
    eim.startEventTimer(300000); // 通关后5分钟强制清场，注意此时无法重连
}

function isTokyoBoss(mob) {
    return mob.getId() == 9400289;  // BOSSID 欧碧拉
}
