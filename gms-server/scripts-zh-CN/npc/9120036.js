/*
	未来东京 BOSS4 努克斯远征队NPC
	适配：北斗GMS083 服务端
	参考：2030013.js 扎昆远征NPC逻辑
	修改：增加地图检测，位于802000111 作为出口，位于802000110 执行远征队功能
*/
...
var expedName = "未来东京 努克斯";
var expedBoss = "努克斯";
var expedMap = "努克斯的所在地";
...
function action(mode, type, selection) {
    ...
    em = cm.getEventManager("Tokyo4");  //调用event脚本名称
    ...
    if (status == 0) {
        // 等级校验（复用远征类型配置，与事件脚本保持一致）
        if (player.getLevel() < 130) {
            cm.sendOk("您的等级不符合挑战" + expedBoss + "的条件！需要至少130级。");
            cm.dispose();
            ...
            if (expedition.contains(player)) { // 已登记玩家可进入
                var eim = em.getInstance("VonLeon" + player.getClient().getChannel());
